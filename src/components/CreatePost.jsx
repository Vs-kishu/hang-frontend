...
export function CreatePost({ postToEdit }) {
  ...
  const wasPostEdited =
    isPostBeingEdited &&
    (postFormData.content !== postToEdit?.content ||
      !areObjectsEqual(postToEdit?.images, localImageUrls));
  return (
    <motion.article
      animate={{ scale: [0.5, 1.1, 1] }}
      transition={{ duration: 0.5, linear: true }}
      className="oerflow-x-hidden relative grid w-full min-w-[95vw] max-w-[99vw] grid-cols-[3rem_auto] gap-2 overflow-visible rounded-2xl bg-light-100 p-4 dark:bg-dark-100 md:min-w-[35rem]"
    >
      <figure>
          ...
          {isPostUploadPending ? (
            ...
            <button
              ...
              className={`rounded-2xl  py-2 px-4 text-primary dark:text-dark-200
                ${
                  !isValidPost || (isPostBeingEdited && !wasPostEdited)
                    ? "cursor-not-allowed bg-gray-400 text-gray-500 dark:bg-gray-700 dark:text-gray-800"
                    : "bg-dark-200 text-primary dark:bg-primary dark:text-dark-200"
                }
              `}
