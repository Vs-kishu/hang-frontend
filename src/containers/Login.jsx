...
export function Login() {
  ...
  const guestLogin = () => {
    formik.setFieldValue("username", "RamJi");
    formik.setFieldValue("password", "ramji34");
    dispatch(
      authenticateUser({
        formData: { username: "RamJi", password: "ramji34" },
        endpoint: "/auth/login",
      })
    );
  };
  const isAuthenticationPending =
    status.value === "pending" && status.type === "authenticateUser";
